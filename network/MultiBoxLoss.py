    ...
    def forward(self, odm_data, priors, targets, arm_data=None, filter_object=False):
        ...
        for idx in range(num):
            ...
            if arm_data:
                ...
                #loc_t[idx, :, :] = truths
                #conf_t[idx, :] = labels
                match(self.threshold, truths, priors.cpu(), self.variance, labels, loc_t, conf_t, idx)
        if GPU:
            loc_t = loc_t.cuda()
            conf_t = conf_t.cuda()
        ...
        loss_l, loss_c = loss_l.double(), loss_c.double()
        loss_l /= N
        loss_c /= N
